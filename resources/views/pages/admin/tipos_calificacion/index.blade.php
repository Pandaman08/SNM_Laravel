...
@section('titulo', 'Gestión de Tipos de Calificación')

@section('contenido')
    <div class="max-w-screen-2xl mx-auto my-8 px-4">
        <div class="text-center mb-6">
            <h1 class="text-2xl font-bold text-[#2e5382]">Tipos de Calificación</h1>
            ...
            <div class="flex space-x-4">
                <form method="GET" class="flex items-center">
                    <input name="buscarpor" class="border rounded-l py-2 px-4" type="search"
                        placeholder="Buscar por código, nombre o descripción" value="{{ $buscarpor }}">
                    <button class="bg-[#98C560] text-white px-4 py-2 rounded-r hover:bg-[#7aa94f]" type="submit">
                        Buscar
                    </button>
                </form>
            </div>
            <a href="{{route('tipos-calificacion.create')}}" class="bg-blue-500 text-white px-6 py-2 rounded hover:bg-blue-700">
                Registrar Tipo
            </a>
        </div>
                ...
                <tbody>
                    @foreach ($tipos as $tipo)
                        <tr class="border-b hover:bg-gray-50">
                            <td class="px-4 py-3 font-medium">{{ $tipo->codigo }}</td>
                            <td class="px-4 py-3">{{ $tipo->nombre }}</td>
                            <td class="px-4 py-3">{{ $tipo->descripcion ?: 'N/A' }}</td>
                            <td class="px-4 py-3 flex items-center justify-center space-x-4">
                                <a href="{{route('tipos-calificacion.edit', $tipo->id_tipo_calificacion)}}"
                                ...
                                <button onclick="openDeleteModal({{ $tipo->id_tipo_calificacion }}, '{{ $tipo->nombre }}')"
        ...
        <div class="flex justify-end text-sm mt-4">
            {{ $tipos->links('pagination::tailwind') }}
        </div>
    </div>
    ...
    <div id="deleteModal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 w-full h-full">
        <div class="flex items-center justify-center w-full h-full">
            <div class="bg-white p-7 rounded shadow-lg max-w-md w-full relative">
                ...
                <h2 class="text-xl font-bold mb-4">Eliminar Tipo de Calificación</h2>
                <p>¿Estás seguro de que deseas eliminar el tipo "<span id="tipoNombre"></span>"?</p>
    ...
    <script>
        function openDeleteModal(id, nombre) {
            ...
            document.getElementById('tipoNombre').innerText = nombre;
            document.getElementById('deleteForm').action = `/tipos-calificacion/${id}`;
        }
