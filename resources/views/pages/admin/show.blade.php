    ...
    <div class="max-w-screen-2xl mx-auto my-8 px-4">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-lg font-semibold text-[#2e5382] flex items-center">
                ...
                <button id="toggleFilters"
                    class="text-[#2e5382] hover:text-[#1e3a5f] transition-colors text-sm font-medium">
                    <span id="toggleText">Mostrar Filtros</span>
                    <svg id="toggleIcon" class="w-4 h-4 inline-block ml-1 transform transition-transform" fill="none"
            ...
            <div id="advancedFilters" class="hidden">
                <form id="filterForm" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <!-- Filtro por Sexo -->
                    <div>
                        <label for="sexoFilter" class="block text-sm font-medium text-gray-700 mb-1">Sexo</label>
                        <select id="sexoFilter" name="sexo"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#2e5382] focus:border-transparent transition-colors">
                    ...
                    <div>
                        <label for="estadoCivilFilter" class="block text-sm font-medium text-gray-700 mb-1">Estado
                            Civil</label>
                        <select id="estadoCivilFilter" name="estado_civil"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#2e5382] focus:border-transparent transition-colors">
                            <option value="">Todos</option>
                            <option value="S" {{ request('estado_civil') == 'S' ? 'selected' : '' }}>Soltero/a</option>
                            <option value="C" {{ request('estado_civil') == 'C' ? 'selected' : '' }}>Casado/a</option>
                            <option value="D" {{ request('estado_civil') == 'D' ? 'selected' : '' }}>Divorciado/a
                            </option>
                            <option value="V" {{ request('estado_civil') == 'V' ? 'selected' : '' }}>Viudo/a</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select id="orderBy" name="order_by"
                            ...
                            <option value="role" {{ request('order_by') == 'role' ? 'selected' : '' }}>Rol</option>
                        </select>
                    </div>

                    <!-- Edad mínima -->
                    <div>
                        <label for="edadMin" class="block text-sm font-medium text-gray-700 mb-1">Edad mínima</label>
                        <input type="number" id="edadMin" name="edad_min" min="18" max="100"
                    ...
                    <!-- Edad máxima -->
                    <div>
                        <label for="edadMax" class="block text-sm font-medium text-gray-700 mb-1">Edad máxima</label>
                        <input type="number" id="edadMax" name="edad_max" min="18" max="100"
                                    ...
                                    <span
                                        ...
                                        @if ($usuario->rol === 'admin')
                                            <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                                ...
                                                <path
                                                    d="M1 8a1 1 0 011-1h1v1a1 1 0 11-2 0V8zM12 8a1 1 0 011-1h1v1a1 1 0 11-2 0V8zM7 12a1 1 0 011-1h1v1a1 1 0 11-2 0v-1z">
                                                </path>
                                            </svg>
                                        @endif
                                        {{ $usuario->rol }}
            ...
            <form action="{{ route('users.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            <label for="estado_civil" class="block text-sm font-medium text-gray-700 mb-1">Estado
                                Civil</label>
                        ...
                        <div>
                            <label for="fecha_nacimiento" class="block text-sm font-medium text-gray-700 mb-1">Fecha
                                Nacimiento *</label>
                ...
                <div class="p-6">
                    ...
                    <form id="editForm" action="" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-4">
                                <div>
                                    <label for="edit_name" class="block text-sm font-medium text-gray-700 mb-1">Nombres
                                        *</label>
                                ...
                                <div>
                                    <label for="edit_dni" class="block text-sm font-medium text-gray-700 mb-1">DNI
                                        *</label>
                                ...
                                <div>
                                    <label for="edit_phone" class="block text-sm font-medium text-gray-700 mb-1">Teléfono
                                        *</label>
                            ...
                            <div class="space-y-4">
                                <div>
                                    <label for="edit_email" class="block text-sm font-medium text-gray-700 mb-1">Email
                                        *</label>
                                ...
                                <div>
                                    <label for="edit_fecha_nacimiento"
                                        class="block text-sm font-medium text-gray-700 mb-1">Fecha Nacimiento *</label>
                                ...
                                <div>
                                    <label for="edit_estado_civil"
                                        class="block text-sm font-medium text-gray-700 mb-1">Estado Civil</label>
            ...
            <div class="bg-white rounded-xl shadow-xl max-w-md w-full">
                <div class="p-6">
                    ...
                    <p class="text-gray-600 mb-6">
                        ¿Estás seguro de que deseas eliminar al usuario "<span id="usuarioNombre"
            ...
            const updateEmailPreview = () => {
                ...
                if (name && lastname) {
                    ...
                    emailPreview.innerHTML =
                        `<strong>Email que se generará:</strong> ${name}${lastname.charAt(0)}_${randomNumbers}@bruning.com`;
...
// Función para aplicar filtros y actualizar toda la sección de resultados
function aplicarFiltros() {
    ...
    // Realizar la solicitud a la ruta principal (no a /buscar)
    fetch(`/users/buscar?${params.toString()}`, {
        method: 'GET',
        headers: {
    ...
    .then(html => {
        ...
        // Actualizar paginación - buscar el contenedor específico
        const newPagination = doc.querySelector('.flex.justify-between.items-center.mt-6');
        if (newPagination) {
            document.querySelector('.flex.justify-between.items-center.mt-6').outerHTML = newPagination.outerHTML;
    ...
    .catch(error => {
        ...
        Swal.fire({
            ...
            text: 'No se pudieron aplicar los filtros. Intente nuevamente.',
            confirmButtonColor: '#2e5382'
        });
    });
}

// Función para adjuntar event listeners a los enlaces de paginación
function attachPaginationListeners() {
    // Manejar clics en enlaces de paginación
    document.querySelectorAll('.pagination a').forEach(link => {
        link.addEventListener('click', function(e) {
            e.preventDefault();
            handlePaginationClick(this.href);
        });
    });
}
...
function handlePaginationClick(url) {
    ...
    .then(html => {
        ...
        // Actualizar toda la sección de paginación
        const newPaginationSection = doc.querySelector('.flex.justify-between.items-center.mt-6');
        if (newPaginationSection) {
            document.querySelector('.flex.justify-between.items-center.mt-6').outerHTML = newPaginationSection.outerHTML;
    ...
    .catch(error => {
        ...
        window.location.href = url; // Fallback a recarga completa
    });
}

// Event listeners para todos los filtros
document.getElementById('roleFilter').addEventListener('change', aplicarFiltros);
document.getElementById('sexoFilter').addEventListener('change', aplicarFiltros);
document.getElementById('estadoCivilFilter').addEventListener('change', aplicarFiltros);
document.getElementById('edadMin').addEventListener('input', debounce(aplicarFiltros, 500));
document.getElementById('edadMax').addEventListener('input', debounce(aplicarFiltros, 500));
document.getElementById('orderBy').addEventListener('change', aplicarFiltros);
document.getElementById('orderDirection').addEventListener('change', aplicarFiltros);
...
// Adjuntar event listeners a la paginación cuando se carga la página
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (urlParams.toString()) {
        // Llenar los campos del formulario con los parámetros de la URL
        ...
        // Aplicar filtros después de un breve delay para asegurar que los campos se llenaron
        setTimeout(aplicarFiltros, 100);
    }
});

// También adjuntar listeners cuando se hace clic en elementos de paginación (delegación de eventos)
document.addEventListener('click', function(e) {
    if (e.target.closest('.pagination a')) {
        e.preventDefault();
        handlePaginationClick(e.target.closest('a').href);
    }
});

// Función de búsqueda original (mantener por compatibilidad)
function buscarUsuarios(query) {
    // Crear parámetros con todos los filtros actuales
    const formData = new FormData(document.getElementById('filterForm'));
    const params = new URLSearchParams();
    ...
    .catch(error => {
        ...
        function openModal(imageUrl, type) {
            ...
            modalContent.innerHTML =
                `<img src="${imageUrl}" class="max-w-full max-h-[70vh] object-contain mx-auto rounded-lg">`;
