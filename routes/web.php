...
// Route::get('/reporte_notas/{codigo_matricula}/{id_asignatura}/create', [ReporteNotasController::class, 'create'])->name('reporte_notas.create');
    ...
    Route::get('/estudiantes/{id}/generate-qr', [AsistenciaQRController::class, 'showQRForm'])
        ->name('asistencia.generate-form');
    Route::post('/estudiantes/{id}/generate-qr', [AsistenciaQRController::class, 'generateQR'])
        ->name('asistencia.generate');
    Route::post('/asistencia/repair-qr/{id}', [AsistenciaQRController::class, 'repairQR'])
    ->name('asistencia.repair-qr');

    // Mostrar QR del estudiante
    Route::get('/estudiantes/{id}/qr', [AsistenciaQRController::class, 'showStudentQR'])
        ->name('asistencia.show-qr');

    // Descargar QR
    Route::get('/estudiantes/{id}/download-qr', [AsistenciaQRController::class, 'downloadQR'])
        ->name('asistencia.download-qr');
    Route::get('/asistencia/qr-image/{id}', [AsistenciaQRController::class, 'showQRImage'])
    ->name('asistencia.qr-image');
    // Escanear QR
    Route::get('/asistencia/scanner', [AsistenciaQRController::class, 'showScanner'])
        ->name('asistencia.scanner');
    Route::post('/asistencia/process-scan', [AsistenciaQRController::class, 'processScan'])
        ->name('asistencia.process-scan');
